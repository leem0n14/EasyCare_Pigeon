...
? define(factory)
    ...
    function createError(message, statusCode, config, request, response) {
      const error = new AxiosError(message, statusCode, config, request, response);
      return error;
    }
    ...
    */
    ...
    const InterceptorManager = function InterceptorManager() {
      this.handlers = [];
    };
    ...
    function mergeConfig(defaultConfig, userConfig) {
      ...
      const defaultToUserConfig = ['baseURL', 'transformRequest', 'transformResponse', 'paramsSerializer', 'timeout', 'withCredentials', 'adapter', 'responseType', 'xsrfCookieName', 'xsrfHeaderName', 'onUploadProgress', 'onDownloadProgress', 'maxContentLength', 'validateStatus', 'maxRedirects', 'httpAgent', 'httpsAgent', 'cancelToken', 'socketPath'];
      ...
      const mergeDeepProperties = ['header', 'auth', 'proxy'];
    ...
    const Axios = function Axios(defaultConfig) {
    ...
    const CancelToken = function CancelToken(executor) {
      const this$1 = this;
      let resolvePromise;
    ...
    function getAxios(config) {
      const context = new Axios(config); // Axios 实例，用于给 axios继承自身属性

      const axios = Axios.prototype.request.bind(context); // 复制原型方法，并绑定函数`this`  Copy axios.prototype to instance
